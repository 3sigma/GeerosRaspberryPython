...
def CalculVitesse():
    ...
    try:
        #codeursDeltaPos = arduino.read_codeursDeltaPos()
        ...
        codeurGaucheDeltaPos = arduino.read_codeurGaucheDeltaPos()
        codeurDroitDeltaPos = arduino.read_codeurDroitDeltaPos()
...
def CommandeMoteurs(commandeDroit, commandeGauche, tensionAlim):
    ...
    tension_int_droit = int(255 * tensionDroit / tensionAlim)
    tension_int_gauche = int(255 * tensionGauche / tensionAlim)

    # Saturation par sécurité
    if (tension_int_droit > 255):
        tension_int_droit = 255

    if (tension_int_droit < -255):
        tension_int_droit = -255

    if (tension_int_gauche > 255):
        tension_int_gauche = 255
    ...
    # Commande PWM
    try:
        arduino.motors(tension_int_gauche, tension_int_droit)    
    except:
